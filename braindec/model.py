    ...
    def __init__(self, hidden_layers=(64, 32, 16), output_channels=1, kernel_size=3, stride=1):
        ...
        for i in range(len(reversed_channels) - 1):
            decoder_layers.extend([
                ...
                self.create_conv_block(reversed_channels[i], reversed_channels[i+1], transpose=True)
            ])
...
class MRI3DAutoencoder(nn.Module):
    def __init__(
        ...
        self.bottleneck = nn.Sequential(
            nn.Linear(bottleneck_size * (input_shape[0] // 8) * (input_shape[1] // 8) * (input_shape[2] // 8), bottleneck_size),
            nn.ReLU(),
            nn.Linear(bottleneck_size, bottleneck_size * (input_shape[0] // 8) * (input_shape[1] // 8) * (input_shape[2] // 8)
        )
